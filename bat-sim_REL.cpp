#include<iostream>
#include<string>
#include"./class/bat_pack/3bat_pack.h"
#include<stdio.h>
#include<iostream>
#include<cstdlib>
#include<pthread.h>
...
/*GNU compiler issue with < 4.8 versions with stof and strof, not working directly on 'string' type, using const char* instead*/
			...
			if(temp_f > batPack.bat[k].get_V_max() )
			{cout << "\n\tV limits::Max voltage for battery is " << batPack.bat[k].get_V_max()<< " setting default max for batt cell" << (k+1);}
			if(temp_f < batPack.bat[k].get_V_cutoff() )
			{cout << "\n\t V limits:: Min volage for battery is " << batPack.bat[k].get_V_cutoff()<< " setting default min for batt cell" << (k+1);}
...
/*	else if((command == "stop"))
	{
		...
		cout << "INVALID Command please refer README.txt file\n";
	}
